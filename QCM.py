...
if st.session_state.screen=='menu':
    ...
    for m in get_modules():
        if st.button(m): st.session_state.update(selected_module=m, screen='lesson'); force_rerun()
    ...
    for lf in get_jsons(mod):
        if col_l.button(lf[:-5]):
            st.session_state.update(selected_lesson=lf, qcm=load_qcm(os.path.join(mod,lf)),
                                    exam_mode=False, screen='mode'); force_rerun()
    col_r.markdown('**Choisis un type d’examen :**')
    if col_r.button('🧪 Exam simili 20 Q'):
        st.session_state.update(qcm=build_exam(mod,20), exam_mode=True, mode='Aléatoire', screen='quiz'); reset_quiz(); force_rerun()
    if col_r.button('🧪 Exam simili 40 Q'):
        st.session_state.update(qcm=build_exam(mod,40), exam_mode=True, mode='Aléatoire', screen='quiz'); reset_quiz(); force_rerun()
